...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    deadFairyCircle.split('').forEach((letter, index) => {
        ...
        if (letter === ' ') {
            ...
            letterElement.classList.add('letter');
            letterElement.id = `letter${welcomeTo.length + index + 1}`;
            const letterImg = document.createElement('img');
            letterImg.src = `${window.location.origin}/shrines/OMORI/omoriAssets/keys/key${letter}.png`;
            letterImg.alt = letter;
            letterImg.style.display = 'none';
        ...
        deadFairyCircleWrapper.appendChild(letterElement);
    });

    keys.forEach(key => {
        const letterIndexes = [];
        for (let i = 0; i < word.length; i++) { // Use the `word` variable here
            if ((i < welcomeTo.length && welcomeTo[i].toUpperCase() === key) ||
                (i >= welcomeTo.length && deadFairyCircle[i - welcomeTo.length].toUpperCase() === key)) {
        ...
        letterIndexes.forEach(index => {
            ...
            if (letterImg) {
                letterImg.style.display = 'inline-block';
            }
    ...
    incorrectKeys.forEach(key => {
        const letterImg = document.createElement('img');
        letterImg.src = `${window.location.origin}/shrines/OMORI/omoriAssets/keys/key${key}.png`;
        letterImg.alt = key;
...
function drawHangman(incorrectCount, uniqueKeys) {
    ...
    if (uniqueKeys.size === 26) {
        ...
        const img = new Image();
        img.src = '../../OMORI/omoriAssets/SomethingL.webp';
        img.onload = function() {
...
function checkAllLettersRevealed() {
    // console.log("Checking if all letters are revealed...");
    const hangmanWordContainer = document.getElementById('hangmanWord');
    ...
    // console.log("Are all letters revealed?", allLettersRevealed);

    if (allLettersRevealed && !isHangmanFunctionCalled) {
        // console.log("All letters are revealed!");
        applyAnimations();
        checkKeys();
    }
}
...
function applyAnimations() {
    ...
    setTimeout(() => {
        allLetters.slice(1).forEach((letter) => {
            letter.classList.add('second-letter-animation');
        });
    }, 720);
    ...
    setTimeout(() => {
        ...
        allLetters.forEach(letter => {
            letter.classList.remove('first-letter-animation');
            letter.classList.remove('second-letter-animation');
        });
    }, 1350);
...
document.addEventListener('DOMContentLoaded', function () {
    var collectedKeys = JSON.parse(localStorage.getItem('collectedKeys')) || [];
