...
function checkKeys() {
    const keys = JSON.parse(localStorage.getItem('collectedKeys')) || [];
    if (localStorage.getItem('keysCollected') === 'true') {
        return;
    }
    ...
    const sortedKeys = keys.map(key => key.toUpperCase()).filter(key => wordLetters.includes(key)).sort();

    // console.log(`Unique letters in word: ${wordLetters}`);
    // console.log(`Sorted collected keys: ${sortedKeys}`);

    if (JSON.stringify(sortedKeys) === JSON.stringify(wordLetters)) {
...
function collectKey(keyId) {
    ...
    if (!keys.includes(keyId)) {
        ...
        function showTextboxContainer() {
            ...
            textbox.innerHTML = `<div id="text"></div><img class="cursor" src="${window.location.origin}/shrines/OMORI/omoriAssets/cursor.png">`;
